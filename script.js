    ...
    embarquesOrdenados.forEach(embarque => {
        ...
        pedidosPorEmbarque[embarque].forEach(pedido => {
            ...
            if (pedidoCarregadoStatus === 'carregado') {
                ...
                statusText = 'Carregamento Concluído';
            ...
            card.innerHTML = `
                <div class="pedido-header">
                    <h3>COT_${pedido.id} - ${pedido.cliente}</h3>
                    <p>Endereço: ${pedido.endereco} <a href="http://googleusercontent.com/maps.google.com/4{encodeURIComponent(pedido.endereco)}" target="_blank" class="map-link">ver no mapa</a></p>
                    <p>NF-e: ${getNotaFiscal(pedido.id) ?? "—"}</p>
    ...
    if (unCheckedItems.length > 0) {
        ...
        confirmarAcao(
            ...
            () => {
                ...
                showModal("Sucesso", `Pedido COT_${pedidoId} marcado como "Carregamento Concluído".`, `<button class="modal-button ok" onclick="closeModal()">OK</button>`);
            }
        );
    }
}
